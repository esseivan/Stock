...
Private Sub btnUpdate_Click()
    ...
    result = StockModule.EditArticleInfos( _
        ...
        mLbl_auto.Value)
    ' Write in history
    WriteHistory_modify mLbl_artN.text, "Article infos updated"
    ' Update filter
...
Private Sub UserForm_Initialize()
    ...
    Set catRange = ws.Range(ws.Cells(s_art_ir_start, s_art_ic_category), ws.Cells(ws.UsedRange.Rows.count, s_art_ic_category))
    
    ' Get all categories and add them if not already done
    ' This item should always be in first pos once sorted
    ListBox3.AddItem ("          ")
    Dim i As Integer
    For i = 1 To ws.UsedRange.Rows.count - 1
        AddValueListbox3 (catRange(i, 1))
    Next
    
    SortListBox ListBox3
    
    ' replace first entry with No Filter
    ListBox3.List(0) = "No filter"
...
Private Function AddValueListbox3(str As String)
    Dim valExists As Boolean
    valExists = False
    Dim i As Integer
    For i = 0 To ListBox3.ListCount - 1
        If ListBox3.List(i) = str Then valExists = True
    Next i
...
Sub ListBox1_Change()
    If (ListBox1.ListIndex >= 0) Then
        ...
        If Not articles_art_nbr Is Nothing Then
            ...
            ' Man. art number
            mLbl_artN.Value = wsa.Cells(Article_Row, StockModule.s_art_ic_art_n).Value
            ' Ret. art number
            ...
            ' Category
            mLbl_cat.Value = wsa.Cells(Article_Row, StockModule.s_art_ic_category).Value
            'Place
...
Private Sub FillArticleList(filter)
    ...
    Set category_range = wsa.Range(wsa.Cells(s_art_ir_start, s_art_ic_category), wsa.Cells(wsa.UsedRange.Rows.count, s_art_ic_category))
    ...
    For i = 2 To articles_range.Rows.count
        ...
        cond2 = False
        ' Get category
        Dim string_c As String
        string_c = category_range.Cells(i - 1, 1).Value
        
        ' Check for filter
        If ListBox3.Selected(0) = True Then
            cond1 = True
        Else
            For j = 1 To (ListBox3.ListCount - 1)
                If (ListBox3.Selected(j)) Then
                    If (string_c = ListBox3.List(j)) Then
                        cond1 = True
                    End If
                End If
            Next
...
Private Sub ListBox3_KeyPress(ByVal KeyAscii As MSForms.ReturnInteger)
    If KeyAscii = 27 Then Unload Me
End Sub
Private Sub mLbl_artN_KeyPress(ByVal KeyAscii As MSForms.ReturnInteger)
    If KeyAscii = 27 Then Unload Me
End Sub
...
Private Sub mLbl_man_KeyPress(ByVal KeyAscii As MSForms.ReturnInteger)
    If KeyAscii = 27 Then Unload Me
End Sub
Private Sub mLbl_cat_KeyPress(ByVal KeyAscii As MSForms.ReturnInteger)
    If KeyAscii = 27 Then Unload Me
End Sub
